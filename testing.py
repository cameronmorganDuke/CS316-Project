...
if __name__ == "__main__":
    query = """
        SELECT LOCATION_ADDR
        FROM dtarp
        WHERE ZONING='PDR 20.000' and CALCULATED_ACRES=4.11
    """
    # query = """
    #     SELECT DISTINCT ZONING, CALCULATED_ACRES
    #     FROM dtarp
    # """
    # query = """
    #     SELECT NEIGHBORHOOD, LOCATION_ADDR
    #     FROM dtarp
    #     WHERE ETJ IS NULL and CITY IS NULL
    # """ 
    
    query_db("instance/master.db", query)
    
    print(estimate_garbage_cost('PDR 20.000', 4.11))
    
    # get_tables('instance/dtarp.db')
    # get_db_schema("instance/dtarp.db")
    # print(estimate_water_cost(5))
